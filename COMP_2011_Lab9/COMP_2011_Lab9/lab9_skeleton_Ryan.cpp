...
Character* create_monster()
{
   // Add your code here
    ...
    monster->location.row = rand()%5 + 1;
    monster->location.col = rand()%5 + 1;
...
{
   // Add your code here
    int x = game->hero.location.row;
    int y = game->hero.location.col;
    x += move_row;
    y += move_col;
    if (x >= MAZE_SIZE || x < 0 || y >= MAZE_SIZE || y < 0)
    {
        cout << "You reached the edge of the board!" << endl;
        return true;
    }
    else
    {
        game->hero.location.row = x;
        game->hero.location.col = y;
        if (game->board[x][y] == nullptr)
        {
            game->board[x][y] = &(game->hero);
            game->board[x - move_row][y - move_col] = nullptr;
            return true;
        }
        else
        {
            if (game->board[x][y]->power > game->hero.power)
                return false;
            else
            {
                game->hero.power += game->board[x][y]->power;
                game->hero.num_collected += 1;
                int i;
                for (i = 0; game->hero.bag[i] != nullptr; i++);
                game->hero.bag[i] = game->board[x][y];
                game->num_monsters -= 1;
                game->board[x][y] = &(game->hero);
                game->board[x - move_row][y - move_col] = nullptr;
                return true;
...
void delete_all_monsters(GameBoard* game)
{
   // Add your code here
    for (int i = 0; i < MAZE_SIZE; i++)
        ...
        {
            if (game->board[i][j] != nullptr && game->board[i][j] != &(game->hero))
            {
                delete game->board[i][j];
                game->board[i][j] = nullptr;
            }
        }
    for (int i = 0; game->hero.bag[i] != nullptr; i++)
    {
